...
\frame { \frametitle{Questionário}
	\begin{block}{Questão 01}
	      Quantas chaves tem um nó não-folha com 3 filhos?
	\end{block}
}
...
\frame { \frametitle{Questionário}
	\begin{block}{Questão 02 -- Resposta Esperada}
		\begin{itemize}
		      \item Máximo: Oito (8) chaves: 2 da raiz, 2 em cada um dos três filhos possíveis.
		      \item Mínimo: Três (3) chaves, pois armazena 2 chaves por nó. % a partir do terceiro nó que a estruturação começa de verdade
		\end{itemize}
	\end{block}
}

\frame { \frametitle{Questionário}
	\begin{block}{Questão 03}
	      Qual o número máximo de comparações de chaves em uma árvore 2-3?
	\end{block}
}

\frame { \frametitle{Questionário}
	\begin{block}{Questão 03 -- Resposta Esperada}
	      É o dobro do número de nós visitados.
	\end{block}
}

\frame { \frametitle{Questionário}
    \begin{block}{Questão 04}
        Qual a vantagem da árvore 2-3 sobre outra árvore de busca?
    \end{block}
}

\frame { \frametitle{Questionário}
    \begin{block}{Questão 04 -- Resposta Esperada}
        Todas as operações de busca sempre serão em tempo logarítmico.
    \end{block}
}

\frame { \frametitle{Questionário}
    \begin{block}{Questão 05}
        Qual a altura máxima que uma árvore 2-3 com 1 bilhão de chaves pode ter?
    \end{block}
}

\frame { \frametitle{Questionário}
    \begin{block}{Questão 05 -- Resposta Esperada}
        Nas árvores 2-3 a altura nunca passará de log N, caso em que todos os nós serão simples, portanto, log 2bi será a altura máxima.
    \end{block}
}
...
\frame { \frametitle{Referências Bibliográficas}
	\begin{enumerate}
     ...
     \item SEDGEWICK, Robert, Algorithms in C, Third Edition : \textit{Chapter 13 -- Balanced Trees}.
	\end{enumerate}
}
